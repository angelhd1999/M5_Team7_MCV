...
def get_mask_and_image(coco, class_name, save_path='./task_b/', only_image=False):
    ...
    return img, coco.annToMask(anns[0]), anns[0]['bbox']
...
def task_b(predictor, coco, class_name, least5_per_class_dict, random_positions_num, save_path):
    ...
    image, mask, _ = get_mask_and_image(coco, class_name, save_path=save_path)
...
def create_image_with_bbox(image, bbox):
    bbox_image = np.zeros_like(image)
    x, y, w, h = [int(coord) for coord in bbox]
    bbox_image[y:y+h, x:x+w] = image[y:y+h, x:x+w]
    return bbox_image

def create_image_with_noise_inside(image, mask, bbox):
    noise_image = np.zeros_like(image)
    x, y, w, h = [int(coord) for coord in bbox]
    noise = np.random.normal(0, 25, (h, w, 3)).astype(np.uint8)
    noise_image[y:y+h, x:x+w] = noise
    noise_image[mask == 1] = image[mask == 1]
    return noise_image

def create_image_with_noise_outside(image, mask, bbox):
    height, width, _ = image.shape
    x, y, w, h = [int(coord) for coord in bbox]
    noise_image = np.zeros_like(image)
    
    # Create noise outside the bounding box
    noise_image[:y, :] = np.random.normal(128, 25, (y, width, 3)).astype(np.uint8)
    noise_image[y+h:, :] = np.random.normal(128, 25, (height-y-h, width, 3)).astype(np.uint8)
    noise_image[y:y+h, :x] = np.random.normal(128, 25, (h, x, 3)).astype(np.uint8)
    noise_image[y:y+h, x+w:] = np.random.normal(128, 25, (h, width-x-w, 3)).astype(np.uint8)
    
    # Make all the pixels inside the bounding box but outside the mask black
    noise_image[y:y+h, x:x+w] = np.where(
        mask[y:y+h, x:x+w][:, :, np.newaxis] == 1,
        image[y:y+h, x:x+w],
        0
    )
...
def task_d(predictor, coco, class_name, save_path):
    # Get the image, mask, and bounding box of the instance
    image, mask, bbox = get_mask_and_image(coco, class_name, save_path=save_path)

    # Save the original image and visualize the prediction
    img_original = f'{save_path}{class_name}.jpg'
    vis_img_original = f'{save_path}{class_name}_vis.jpg'
    plt.imsave(vis_img_original, run_inference_and_visualize(predictor, img_original))

    # Create images with different modifications
    masked_image = create_image_with_mask(image, mask)
    bbox_image = create_image_with_bbox(image, bbox)
    noise_image_inside = create_image_with_noise_inside(image, mask, bbox)
    noise_image_outside = create_image_with_noise_outside(image, mask, bbox)

    # Save the images with modifications and visualize their predictions
    for i, modified_image in enumerate([masked_image, bbox_image, noise_image_inside, noise_image_outside]):
        img_name = f'{save_path}{class_name}_modified_{i+1}.jpg'
        vis_img_name = f'{save_path}{class_name}_modified_{i+1}_vis.jpg'
        plt.imsave(img_name, modified_image)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--task_d', action='store_true', help="Perform Task D.")
    ...
    if DO_TASK_D:
        # Create a folder to save the images using python, add a timestamp to the folder name
        experiment_path = f'./task_d/{timestamp}_{class_name}/'
        os.mkdir(experiment_path)    
        task_d(predictor, coco, class_name, experiment_path)